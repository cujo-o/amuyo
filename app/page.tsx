...
const FloodVisualizer = dynamic(() => import("@/components/FloodVisualizer"), { ssr: false });
    ...
    <main className="min-h-screen bg-[#070709] text-gray-100 font-sans pb-12">
      ...
      <nav className="flex items-center justify-between px-6 py-4 border-b border-white/10 bg-[#0c0c0f] sticky top-0 z-50">
        <div>
          ...
          <p className="text-[10px] text-gray-400 font-mono">Crowdsourced Flood Defense</p>
        </div>

        <div className="relative">
          <button onClick={() => setIsLangModalOpen(!isLangModalOpen)} className="flex items-center gap-2 px-3 py-1.5 rounded-lg bg-white/5 border border-white/10 text-xs font-mono hover:bg-white/10">
        ...
        <div className="lg:col-span-5 flex flex-col gap-6">
          <div className="bg-[#111115] rounded-2xl border border-white/10 p-5 shadow-xl">
            ...
            <label className="block relative w-full h-44 rounded-xl border-2 border-dashed border-white/15 hover:border-blue-500 cursor-pointer overflow-hidden group">
              ...
              {previewUrl ? <img src={previewUrl} className="object-cover w-full h-full group-hover:scale-105 transition-transform" /> : <div className="absolute inset-0 flex flex-col items-center justify-center text-gray-400"><span className="text-3xl">📱</span></div>}
            </label>

            <button onClick={handleUpload} disabled={loading || !file} className="w-full mt-4 py-3.5 bg-blue-600 hover:bg-blue-500 text-white rounded-xl text-xs font-bold uppercase transition-all disabled:opacity-40 flex justify-center gap-2">
              {loading ? <span className="w-4 h-4 border-2 border-white/30 border-t-white rounded-full animate-spin" /> : null}
              {loading ? t.analyzingBtn : t.analyzeBtn}
            </button>
            {errorMessage && <p className="mt-3 text-xs text-red-400 font-mono">Error: {errorMessage}</p>}
          </div>
          ...
          {analysisData && (
            <div className="bg-[#111115] rounded-2xl border border-red-900/50 p-5 shadow-xl animate-in fade-in">
              <div className="flex justify-between mb-3">
                <span className="text-xs font-bold text-red-400 uppercase">⚠️ {t.threatTitle}: {analysisData.status}</span>
                <button onClick={handleAudioBroadcast} className="px-2.5 py-1 bg-red-950 border border-red-800 text-red-200 text-xs rounded-lg font-mono">🔊 {t.speakBtn}</button>
              </div>
              <div className="p-3.5 bg-black/40 rounded-xl text-sm text-gray-200">{analysisData.alerts[language]}</div>
            </div>
          )}
        </div>
        ...
        <div className="lg:col-span-7 flex flex-col gap-6">
          <div className="bg-[#111115] rounded-2xl border border-white/10 overflow-hidden flex flex-col h-[460px]">
            <div className="flex border-b border-white/10 bg-[#0a0a0d]">
              <button onClick={() => setActiveTab("3D")} className={`flex-1 py-3.5 text-xs font-bold uppercase ${activeTab === "3D" ? "text-blue-400 border-b-2 border-blue-500 bg-[#111115]" : "text-gray-500"}`}>{t.tab3D}</button>
              <button onClick={() => setActiveTab("MAP")} className={`flex-1 py-3.5 text-xs font-bold uppercase ${activeTab === "MAP" ? "text-blue-400 border-b-2 border-blue-500 bg-[#111115]" : "text-gray-500"}`}>{t.tabMap}</button>
            </div>
            <div className="flex-1 relative">
              {activeTab === "3D" ? <FloodVisualizer data={analysisData} imageTextureUrl={previewUrl} /> : <HazardMap data={analysisData} />}
            </div>
          </div>

          {analysisData && (
            <div className="bg-[#111115] rounded-2xl border border-white/10 p-5 space-y-4 animate-in fade-in">
              <h3 className="text-xs font-bold text-blue-400 uppercase">{t.aiReasoning}</h3>
              <div className="space-y-2 text-xs font-mono text-gray-300 bg-black/40 p-4 rounded-xl border border-white/5">
                <div><span className="text-blue-400">1.</span> {analysisData.reasoningChain.visualBenchmark[language]}</div>
                <div><span className="text-blue-400">2.</span> {analysisData.reasoningChain.hydrodynamicForces[language]}</div>
                <div><span className="text-blue-400">3.</span> {analysisData.reasoningChain.predictiveEvacuationWindow[language]}</div>
              </div>

              <div>
                <h4 className="text-xs font-bold text-gray-400 mb-2">{t.fieldActions}</h4>
                <ul className="space-y-1.5 text-xs font-mono text-cyan-200">
                  {analysisData.tacticalActionPlan[language].map((action, i) => (
                    <li key={i} className="flex gap-2"><span className="text-blue-400">✓</span><span>{action}</span></li>
                  ))}
                </ul>
              </div>
            </div>
